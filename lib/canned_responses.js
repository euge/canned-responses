...
CannedResponses.prototype.middleware = function(req, res, next) {
  this.load(function() {
    ...
    var status = req.get("X-Response-Status") || 200;
    var responseHeaders = JSON.parse(req.get("X-Response-Headers") || "{}");
    var body = req.get("X-Response-Body");

    if (response) {
      if (body) {
        // do nothing, use the body from the request header
